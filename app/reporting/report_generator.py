...
""".strip()
    ...
    def save_pdf(self, html: str, output_path: str) -> None:
        ...
        Imports xhtml2pdf lazily so the app can still start even if PDF dependency
        is missing; in that case user gets a clear runtime error when exporting PDF.
        """
        try:
            from xhtml2pdf import pisa
        except ModuleNotFoundError as exc:
            raise RuntimeError(
                "Модуль xhtml2pdf не установлен. Выполните: pip install xhtml2pdf"
            ) from exc
